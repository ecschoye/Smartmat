<template>
    <div class="h-1/2 py-6 flex flex-col items-center justify-center sm:py-12 overflow-hidden">
      ...
      />
      <div id="listwrapper" class="relative overflow-hidden">
        <ul v-if="open" @click="open = !open" class="w-1/2 h-72">
          <div v-for="grocery in filteredGroceries" :key="grocery.id">
            <li
              @click="setGrocery(grocery)"
              :class="{ 'bg-blue-200': grocery.id === selectedGrocery?.id && grocery.name === selectedGroceryName }"
              class="w-full text-gray-700 p-4 mt-2 bg-white"
            >
              {{ grocery.name }}
            </li>
          </div>
  ...
  const filteredGroceries = computed(() => {
    if (selectedGrocery.value !== null) {
      return [
        ...
        ...groceries.filter((grocery) => grocery.id !== selectedGrocery.value!.id),
      ];
    } else {
      return groceries;
    }
